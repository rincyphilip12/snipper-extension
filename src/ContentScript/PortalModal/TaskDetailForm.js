...
function TaskDetailForm({
  ...
  const [taskListLoader, setTaskListLoader] =
    useState(false);
      ...
      chrome.runtime.sendMessage(
        ...
        res => {
          if (res.STATUS == "OK") {
            ...
            throw new Error("Error in project dropdown");
        }
      );
    } catch (e) {
      setTaskListLoader(false);
  ...
  const createSubTaskHandler = async e => {
    ...
    try {
      ...
      chrome.runtime.sendMessage(
        {
          cmd: "FETCH",
          url: `tasks/${selectedTaskId?.current}.json`,
          method: "POST",
        ...
        res => {
          if (res?.STATUS === "OK") {
            showLoader(false);
            showToastMessage(
              "Subtask has been created succesfully",
              true
            );
