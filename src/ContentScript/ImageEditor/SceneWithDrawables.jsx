...
import { ArrowDrawable, CircleDrawable,TextDrawable } from './Drawables';
import URLImage from "./URLImage";
import { useRef, useState } from "react";

function SceneWithDrawables({ uploadScreenshot, dataUri, coords }) {

    const [drawables, setDrawables] = useState([]);
    const [newDrawable, setNewDrawable] = useState([]);
    const [newDrawableType, setNewDrawableType] = useState('ArrowDrawable');
    ...
    const handleMouseUp = e => {
        ...
        if (newDrawable.length === 1) {
            ...
            setNewDrawable([]);
            setDrawables((oldDrawables => [...oldDrawables, drawableToAdd]))
        }
    };
    ...
    return (
        ...
        <div>
            <button onClick={e => setNewDrawableType("ArrowDrawable")}>
                Draw Arrows
            </button>
            <button onClick={e => setNewDrawableType("CircleDrawable")}>
                Draw Circles
            </button>
            <button onClick={e => setNewDrawableType("TextDrawable")}>
                Add Text
            </button>
            <button onClick={onUndoDrawables}>
                Undo!
            </button>
            ...
            >
                <Layer>
                    {/* CLIPPED IMAGE */}
                    <URLImage src={dataUri} coords={coords} setStageDimensions={setStageDimensions} />

                    {/* DRAWABLES  */}
                    {drawables[0] && [...drawables, ...newDrawable].map(drawable => {
